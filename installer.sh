...
# 3. Publish RIL as a self-contained executable
echo "Publishing RIL..."
...
# 5. Create directory /usr/local/bin/RIL
sudo mkdir -p /usr/local/bin/RIL

# 6. Copy executable into /usr/local/bin/RIL
sudo cp "$EXE_PATH" /usr/local/bin/RIL/ren
sudo chmod +x /usr/local/bin/RIL/ren

# 7. Create symlink to /usr/local/bin so command 'ren' works
sudo ln -sf /usr/local/bin/RIL/ren /usr/local/bin/ren

echo "=== Installation complete! ==="
echo "You can now run 'ren' from anywhere."
echo "Executable is located in /usr/local/bin/RIL/"
